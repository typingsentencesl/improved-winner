...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + 0 * 2.0f));
    ...
    PushStyleColor(ImGuiCol_Border, IM_COL32(45, 45, 45, 255));
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
    if (*v)
    {
        const float padding = 1.0f;
        RenderFrame(ImVec2(check_bb.Min.x + padding, check_bb.Min.y + padding), ImVec2(check_bb.Max.x - padding, check_bb.Max.y - padding), GetColorU32(ImGuiCol_CheckMark), true, style.FrameRounding);
    }
...
void RenderModernArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float scale)
{
    ...
    draw_list->AddLine(center + a + offset, center + b, col, 1.5f);
    draw_list->AddLine(center + a - offset, center + c, col, 1.5f);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : GetContentRegionAvail().x;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 20));
    const ImRect frame_bb(bb.Min + ImVec2(0, label_size.y + 5), bb.Max);
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, id, &frame_bb))
        return false;
    ...
    if (hovered)
    {
        if (label_size.x > 0) RenderText(bb.Min, label);
        if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview)) RenderTextClipped(frame_bb.Min + ImVec2(5.0f, 0), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    }
    else
    {
        ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(155, 155, 155, 255));
    ...
    // arrow
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size - 5.0f);
    if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
    {
        if (hovered)
        {
            RenderModernArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.x, frame_bb.Min.y + style.FramePadding.y), ImColor(255, 255, 255, 255), ImGuiDir_Down, 1.0f);
        }
        else
        {
            RenderModernArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.x, frame_bb.Min.y + style.FramePadding.y), ImColor(155, 155, 155, 255), ImGuiDir_Down, 1.0f);
        }
    }
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(style.FramePadding.x, 5.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(clickable, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), clickable.Max - ImVec2(0.f, 7.f), ImGui::GetColorU32(ImGuiCol_FrameBg));
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(178, 102, 255, 255));
    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_anim->second.circle_anim, ImColor(178, 102, 255, 255), 30.f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered)
    {
        RenderTextClipped(bb_inner.Min + ImVec2(5.0f, 0), bb.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f));
    }
    else
    {
        ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(155, 155, 155, 255));
